...
class BlogClient(BaseClient):
    ...
    def get_draft_posts(self, blog_guid, **options):
        return self._call('{}/posts.json'.format(blog_guid), params={'draft': 'true'}, **options)
    ...
    def get_pulished_posts(self, blog_guid, **options):
        return self._call('{}/posts.json'.format(blog_guid), params={'draft': 'false'}, **options)
    ...
    def update_post(
        ...
        **options
        ...
        post_dict = dict(
            [(k, locals()[p]) for p, k in update_param_translation.items() if locals().get(p)])
