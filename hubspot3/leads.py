...
class LeadsClient(BaseClient):
    ...
    def _get_path(self, subpath):
        return 'leads/v{}/{}'.format(self.options.get('version') or LEADS_API_VERSION, subpath)
    ...
    def update_lead(self, guid, update_data=None, **options):
        ...
        return self._call('lead/{}/'.format(guid), data=update_data, method='PUT', **options)
    ...
    def close_lead(self, guid, close_time=None, **options):
        return self.update_lead(
            guid,
            {'closedAt': close_time or int(time.time() * 1000)},
            **options
        )
